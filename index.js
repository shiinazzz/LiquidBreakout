            ...
            .then(async res => {
              ...
              if (!isOnSale) reject("Item is not on-sale.");
                  ...
                  axios({
                    ...
                    .catch(res => reject(`Failed to whitelist, error code: ${res.response != null ? `${res.response.status}\nResponse: ${res.response.data}` : "Unknown. Token got changed?"}`))
                } else resolve(`${assetId} is already whitelisted.`);
            })
            .catch(res => { console.log(res); reject(`Failed to fetch information. Error code: ${res.response != null ? `${res.response.status}\nResponse: ${res.response.data}` : "Unknown."}\nMessage: ${res.data && res.data.errors ? res.data.errors[0].message : res.message ? res.message : res}`) } )
